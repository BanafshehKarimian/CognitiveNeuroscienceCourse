function [liklihood,P,Q] = VS2(data, beta, alphap, alphai, alpha_inv, k,p)
    ...
    for i = 1:length(trial)
       if ~(ID(i)=="dem")%is not a demonstrating trial
           ...
           tau = (Q(state(i),d) == max(Q(state(i),:)));
           alphai(state(i)) = alphai(state(i)) + 0.1*(tau-alphai(state(i)));
           Q(state(i),d) = Q(state(i),d) + alphai(state(i))*(1-Q(state(i),d));
       end
    end
end