function [liklihood,P,Q] = MB7(data, beta, alphap, alphai, alpha_inv, k,p)
    ...
    for i = 1:length(trial)
       if ~(ID(i)=="dem")%is not a demonstrating trial
           ...
           P(state(i),d) = P(state(i),d) + alphai*(1-P(state(i),d));
           ...
           QD(state(i),dn) = QD(state(i),dn) + alpha_inv*(-1-QD(state(i),dn));
       end
    end
end